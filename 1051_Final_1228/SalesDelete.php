...
if(isset($_POST["ID"]))
{	 
	 $sql_query = "DELETE FROM sales WHERE ID = ".$_POST["ID"];
	 $delete_result = mysqli_query($db, $sql_query);
	# print($delete_result);
	$spl_stock = "SELECT number AS numDelete FROM sales WHERE ID =".$_POST["ID"];
	$spl = "SELECT ISBN FROM sales WHERE ID =".$_POST["ID"];
	$sql_query = "UPDATE stock SET inventory_level = inventory_level - '".$spl_stock."' WHERE ISBN = '".$spl."'";
	mysqli_query($db, $sql);
			...
			<form name="mfrom" method="post">
				<center>
					<table  border="1" class="table table-hover" style="border:4px lightskyblue solid; width:20%;" rules="all">
						...
						<tbody>
						<tr> 
						  <td scope="row"><select name="ID" style="width:100%;">
　								<?php
									include "dbFinal_conn.php";
